  ...
  return (<>
    ...
    <div
      ...
      {names.map((item: any, index: number) =>
       (<InView as="div" key={`${Object.keys(item)}`} id={`${Object.keys(item)}`} onChange={(inView, entry) => setSection(`${Object.values(item).map((item:any)=>Object.values(item)[0])}`)}><SDiv  props={{
        height: "200vh",
        children: (
          <>
            <h1>{`${Object.values(item).map((item:any)=>Object.values(item)[0])}`}</h1>
            <p className="mr-auto my-4">{`${Object.values(item).map((item:any)=>Object.values(item)[1])}`}</p>
          </>
        )
      }}
      />
